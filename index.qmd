...
Often associated with satellite imagery (gridded observations where each pixel represents a measurement at regular spatial intervals)
...
In this example, the column headers are values (species names) rather than variable names. We are turning `arctic_tern` and `glaucous_gull` into two separate columns: `species` and `count`.
...
Like the `coordinates` column in the example below. We can separate the `coordinates` column into two columns: `longitude` and `latitude`.
...
## Mapping the tidy data

Once the data is tidy, it is easy to visualize it. We can use the coordinates to plot the data on a map and choose a better projection.
...
p1 <- ggplot() +
  ...
  geom_sf(data = species_clean_sf, color = "red") +
  ggrepel::geom_text_repel(
    data = species_clean_sf,
    aes(label = station_name, geometry = geometry),
    stat = "sf_coordinates",
    size = 3L,
    fontface = "bold",
    max.overlaps = 100L,
  ...
  theme(
    ...
    axis.text = element_text(size = 6L),
    panel.grid = element_line(linewidth = 0.1, color = "#3c3c3c")
  )
...
- Data can be reused in other studies (promotes scientific collaborations)
...
- If you have any questions regarding your data, do not hesitate to contact your designated data scientist at your institution.
